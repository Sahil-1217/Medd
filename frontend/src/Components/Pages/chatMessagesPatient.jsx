...
function ChatMessages({ socket}) {
  ...
  useEffect(() => {
    const fetchDoctorList = async () => {
      try {
        const response = await axios.get(`https://medd-9.onrender.com/patient/getDoctorUsername/${username}`);
  ...
  const handleDoctorClick = async (doctorUsername) => {
    try {
      const response = await axios.post(`https://medd-9.onrender.com/patient/Chat/${username}/${doctorUsername}`);
      console.log(response.data)
  ...
  const sendMessage = async () => {
    if (currentMessage !== "") {
      ...
      const response = await axios.post(`https://medd-9.onrender.com/patient/sendMessage/${username}/${selectedDoctor}`, {
        message: currentMessage
      });
      console.log("mariomaaa")
