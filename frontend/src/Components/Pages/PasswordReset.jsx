...
function OTP() {
  ...
  const checkOTP = async () => {
    try {
      const response = await axios.post("https://medd-9.onrender.com/checkOTP", { otp, email });
  ...
  const resetPass = async () => {
    try {
      const response = await axios.post("https://medd-9.onrender.com/resetPass", { newPassword, email });
      console.log(response.data);
      if (response.data.success) {
